...
export default function ShoppingListUnlisted({refreshListed, toRefreshUnlisted}) {
  ...
  const [entireListExpectedCost, setEntireListExpectedCost] = useState('');
  ...
  const basicQuery = async () => {
    ...
    if (user){
      ...
      for (let i = 0; i < results.length; i++){
        ...
        entireListExpectedCost0 += results[i].attributes.itemRetailPrice
      }
      //console.log("resultsToSet", resultsToSet)
      setResultss(resultsToSet)
      console.log("entireListExpectedCost0", entireListExpectedCost0)
    ...
    <div className="flex flex-col items-center">
      ...
      <div className="flex flex-col items-center">{isAuthenticated && (
        ...
        <div className="text-black pt-3 pb-3 text-center">
          Your Shopping List Unlisted Items:
        </div>
