...
import ShoppingListItemCardPublic from "../components/ShoppingListItemCardPublic";
...
export default function SharingList() {
  ...
  async function returnAllResults() {
    const params = {}
    const results = await Moralis.Cloud.run("returnAllShoppingLists", params)
    ...
    console.log("resultsToSet", resultsToSet[0])
        //return
    setResultss(resultsToSet)
    }
      ...
      <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1" />
        ...
        {close => (
          <div className="rounded-xl border-2 border-black bg-white flex flex-col items-center w-[90vw] md:w-[500px]">
            ...
            {alertMessage == "Please Login to contact user!" &&
      ...
      <div>
          <div className="w-[100vw] justify-items-center grid grid-cols-2 gap-[0.5vw] md:w-auto md:gap-[2vw] md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5">
              {resultss.map((card, index) => (
                  <ShoppingListItemCardPublic key={index} {...card} />
              ))}
          </div>
      </div>
